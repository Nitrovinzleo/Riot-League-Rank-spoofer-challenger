...
(function () {
  ...
  // Petite fonction pour extraire "GRANDMASTER" de ".../GRANDMASTER.png"
  function getTierFromBanner(url) {
    if (!url) return 'GRANDMASTER';
  ...
  if (originalFetch && !window._rankModifHookedGB) {
    window._rankModifHookedGB = true;
    window.fetch = async function (...args) {
      ...
      if (url.includes('/lol-ranked/v1/ranked-stats/') || url.includes('/lol-ranked/v1/current-ranked-stats')) {
        try {
          ...
          if (conf) {
            ...
            // 1. Update Solo/Duo entry in queueMap (Laisse Flex, TFT, etc. intacts)
            ...
            // 3. Update highestRankedEntry (pour que le crest principal affiche Grandmaster)
      ...
      if (url.includes('/lol-regalia/v2/')) {
        try {
          const clone = response.clone();
          const data = await clone.json();
          const conf = getPageAuth();
          if (conf) {
            data.highestRankedTier = conf.tier.toUpperCase();
            data.preferredBannerType = "ranked";
            data.preferredCrestType = "ranked";
            data.selectedRegaliaType = "ranked";
            return new Response(JSON.stringify(data), {
              status: response.status,
              statusText: response.statusText,
              headers: response.headers
    ...
    tooltipContainers.forEach(container => {
      ...
      queueBlocks.forEach(queue => {
        ...
        if (emblemElement) {
          ...
          emblemElement.setAttribute('prestige-crest-id', '23');
  ...
  function patchBannersAndCrests(root = document) {
    root.querySelectorAll('lol-regalia-crest-v2-element, lol-regalia-banner-v2-element, lol-regalia-profile-v2-element, lol-regalia-hovercard-v2-element, img.regalia-banner-asset-static-image').forEach(el => {
      const c = getContextConf(el) || getPageAuth();
      if (c) {
        if (el.tagName === 'IMG') {
          if (!el.src.includes(c.tier.toLowerCase()) && !el.src.includes('GRANDMASTER')) el.src = c.banner;
        } else {
          const tU = c.tier.toUpperCase();
          if (el.getAttribute('ranked-tier') !== tU || el.getAttribute('prestige-crest-id') !== '23') {
            el.setAttribute('ranked-tier', tU);
            el.setAttribute('ranked-division', c.div || " ");
            el.setAttribute('crest-type', 'ranked');
            el.setAttribute('banner-type', 'ranked');
            ...
            if (el.shadowRoot) {
              el.shadowRoot.querySelectorAll('[ranked-tier]').forEach(child => child.setAttribute('ranked-tier', tU));
              el.shadowRoot.querySelectorAll('.regalia-crest, .regalia-emblem').forEach(child => child.setAttribute('ranked-tier', c.tier.toLowerCase()));
              el.shadowRoot.querySelectorAll('img').forEach(img => {
                if (img.src && !img.src.includes(c.banner) && (img.src.includes('BannerSkins') || img.src.includes('regalia') || img.classList.contains('regalia-banner-asset-static-image'))) {
                  img.src = c.banner;
                }
              });
            }
